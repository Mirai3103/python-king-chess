        ...
        for dx, dy in directions:
            ...
            while True:
                ...
                if board[x][y].pieceType == PieceType.EMPTY:
                    move = InternalMove(CellName.from_2d(fromX, fromY), CellName.from_2d(x, y), PieceType.ROOK, Piece(), piece, MoveType.NORMAL)
                    if not game.simulate_move(move).is_check(piece.pieceColor):
                        moves.append(move)
                else:
                    if board[x][y].pieceColor != piece.pieceColor:
                        move = InternalMove(CellName.from_2d(fromX, fromY), CellName.from_2d(x, y), PieceType.ROOK, board[x][y], piece, MoveType.CAPTURE)
                        if not game.simulate_move(move).is_check(piece.pieceColor):
                            moves.append(move)
                    break
