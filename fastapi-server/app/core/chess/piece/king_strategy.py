...
class KingStrategy(PieceStrategy):
    def get_moves(self, game: IChess, from_2d: tuple[int, int]) -> list[InternalMove]:
        ...
        for dx, dy in directions:
            ...
            if 0 <= toX < 8 and 0 <= toY < 8 and (board[toX][toY].pieceType == PieceType.EMPTY or board[toX][toY].pieceColor != piece.pieceColor):
                ...
                if not game.simulate_move(move).is_check(piece.pieceColor):
                    moves.append(move)
        if game._castling[piece.pieceColor]['K'] and board[fromX+ 2][fromY ].pieceType == PieceType.EMPTY and board[fromX + 1][fromY].pieceType == PieceType.EMPTY :
        ...
        if game._castling[piece.pieceColor]['Q'] and board[fromX- 2][fromY ].pieceType == PieceType.EMPTY and board[fromX - 2][fromY].pieceType == PieceType.EMPTY and board[fromX- 1][fromY ].pieceType == PieceType.EMPTY:
